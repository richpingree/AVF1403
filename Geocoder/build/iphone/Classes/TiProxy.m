...
//Rare exceptions to indicate a bug in the _geocoder code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_geocoder = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_geocoder]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.geocoder.module"
